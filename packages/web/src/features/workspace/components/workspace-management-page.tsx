...
import { useWorkspaceListQuery, useWorkspaceMembersQuery } from '../hooks/use-workspace-query';
...
export function WorkspaceManagementPage(): React.JSX.Element {
  ...
  function setErrorMessage(error: unknown, fallback: string) {
    setFeedback({ type: 'error', message: getWorkspaceErrorMessage(error, fallback) });
  }
  ...
  async function handleCreateWorkspace(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (name.length === 0) {
      setFeedback({ type: 'error', message: '워크스페이스 이름을 입력해주세요.' });
      return;
    }
  ...
  async function handleUpdateWorkspace(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!selectedWorkspace) {
      setFeedback({ type: 'error', message: '수정할 워크스페이스를 선택해주세요.' });
      return;
    }
  ...
  async function handleCreateInvite(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!selectedWorkspace) {
      setFeedback({ type: 'error', message: '초대할 워크스페이스를 선택해주세요.' });
      return;
    }
